Public Class FrmApprovalPD_KK
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("User") = "" Then
            ...
            Exit Sub
        Else
            If CheckAkses1(Session("User").ToString.Split("|")(1), "ApprovalPD_KK") = False Then
                Response.Redirect("Default.aspx")
                Exit Sub
            End If
        End If
    ...
    Private Sub BindGrid()
        ...
        With CmdGrid
            ...
            If CbViewMix.Checked = True Then
                .CommandText = "SELECT * FROM PdHdr WHERE JobNo=@P1 AND Alokasi=@P2 AND NoPD LIKE '%MIX%' " + GetFilter1() + GetFilter2() + " AND ApprovedByAK IS NOT NULL ORDER BY NoPD DESC"
            Else
    ...
    Private Sub GridPD_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridPD.RowCommand
                ...
                Using RsFind As Data.SqlClient.SqlDataReader = CmdFind.ExecuteReader
                    If RsFind.Read Then
                        If String.IsNullOrEmpty(RsFind("ApprovedByKK").ToString) = False Then
                            xSession = "SEE_APPROVALPD_KK|"
                        Else
                            xSession = "UPD_APPROVALPD_KK|"
                        End If
                    End If
                End Using
            End Using

            xSession = xSession & DDLJob.Text & "|" & SelectRecord.Cells(0).Text & "|" & "FrmApprovalPD_KK.aspx|"
        ...
        ElseIf e.CommandName = "BtnApprove" Then
                ...
                Using RsFind As Data.SqlClient.SqlDataReader = CmdFind.ExecuteReader
                    If RsFind.Read Then
                        If String.IsNullOrEmpty(RsFind("RejectBy").ToString) = False Then
                            ...
                            Exit Sub
                        End If
                        'If RsFind("Alokasi") = "B" Then
                        '    If String.IsNullOrEmpty(RsFind("ApprovedByKT").ToString) = False Then
                        '        LblErr.Text = SelectRecord.Cells(0).Text & " sudah di-Approved oleh Koord. Teknik."
                        '        ErrMsg.ShowOnPageLoad = True
                        '        Exit Sub
                        '    End If
                        'Else
                        '    If String.IsNullOrEmpty(RsFind("ApprovedByDK").ToString) = False Then
                        '        LblErr.Text = SelectRecord.Cells(0).Text & " sudah di-Approved oleh Direksi Keuangan."
                        '        ErrMsg.ShowOnPageLoad = True
                        '        Exit Sub
                        '    End If
                        'End If
            ...
            Using CmdFind As New Data.SqlClient.SqlCommand
                ...
                Using RsFind As Data.SqlClient.SqlDataReader = CmdFind.ExecuteReader
                    If RsFind.Read Then
                        LblErr.Text = SelectRecord.Cells(0).Text & " sudah ada pembayaran."
                        ErrMsg.ShowOnPageLoad = True
                        Exit Sub
                    End If
                End Using
    ...
    Protected Sub BtnDel_Click(sender As Object, e As EventArgs) Handles BtnDel.Click
        ...
        With CmdEdit
            .Connection = Conn
            .CommandType = CommandType.Text
            .CommandText = "UPDATE PdHdr SET ApprovedByKK=@P1,TimeApprovedKK=@P2,ApprovedByTBP=@P3,TimeApprovedTBP=@P4," & _
                           "ApprovedByKT=@P5,TimeApprovedKT=@P6,ApprovedByDP=@P7,TimeApprovedDP=@P8,ApprovedByDK=@P9,TimeApprovedDK=@P10 " & _
                           "WHERE NoPD=@P11"
            ...
            .Parameters.AddWithValue("@P3", If(Action = "UnApprove", DBNull.Value, Session("User").ToString.Split("|")(0)))
            .Parameters.AddWithValue("@P4", If(Action = "UnApprove", DBNull.Value, Now))
            .Parameters.AddWithValue("@P5", If(Action = "UnApprove", DBNull.Value, Session("User").ToString.Split("|")(0)))
            .Parameters.AddWithValue("@P6", If(Action = "UnApprove", DBNull.Value, Now))
            .Parameters.AddWithValue("@P7", If(Action = "UnApprove", DBNull.Value, Session("User").ToString.Split("|")(0)))
            .Parameters.AddWithValue("@P8", If(Action = "UnApprove", DBNull.Value, Now))
            .Parameters.AddWithValue("@P9", If(Action = "UnApprove", DBNull.Value, Session("User").ToString.Split("|")(0)))
            .Parameters.AddWithValue("@P10", If(Action = "UnApprove", DBNull.Value, Now))
            .Parameters.AddWithValue("@P11", NoPD)
            .ExecuteNonQuery()
            .Dispose()
        End With
