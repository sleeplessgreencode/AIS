Public Class FrmApprovalPD_DP
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("User") = "" Then
            ...
            Exit Sub
        Else
            If CheckAkses1(Session("User").ToString.Split("|")(1), "ApprovalPD_DP") = False Then
                Response.Redirect("Default.aspx")
                Exit Sub
            End If
        End If
    ...
    Private Sub BindAlokasi()
        ...
        Using CmdFind As New Data.SqlClient.SqlCommand
            With CmdFind
                .Connection = Conn
                .CommandType = CommandType.Text
                .CommandText = "SELECT Alokasi,Keterangan FROM Alokasi WHERE Alokasi='B'"
            End With
            Dim RsFind As Data.SqlClient.SqlDataReader = CmdFind.ExecuteReader
            While RsFind.Read
    ...
    Private Sub BindGrid()
        ...
        With CmdGrid
            .Connection = Conn
            .CommandType = CommandType.Text
            .CommandText = "SELECT * FROM PdHdr WHERE JobNo=@P1 AND Alokasi=@P2 " + GetFilter1() + GetFilter2() + " AND ApprovedByKT IS NOT NULL ORDER BY NoPD DESC"
    ...
    Private Sub GridPD_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridPD.RowCommand
        If e.CommandName = "BtnUpdate" Then
            ...
            If SelectRecord.Cells(9).Text = "&nbsp;" Then
                xSession = "UPD_APPROVALPD_DP|" & DDLJob.Text & "|" & SelectRecord.Cells(0).Text & "|" & "FrmApprovalPD_DP.aspx|"
            Else
                xSession = "SEE_APPROVALPD_DP|" & DDLJob.Text & "|" & SelectRecord.Cells(0).Text & "|" & "FrmApprovalPD_DP.aspx|"
            End If
                    ...
                    Using RsFind As Data.SqlClient.SqlDataReader = CmdFind.ExecuteReader
                        If RsFind.Read Then
                            If String.IsNullOrEmpty(RsFind("RejectBy").ToString) = False Then
                                ...
                                Exit Sub
                            End If
                            If String.IsNullOrEmpty(RsFind("ApprovedByTBP").ToString) = False Then
                                LblErr.Text = SelectRecord.Cells(0).Text & " sudah di-Approved oleh TBP."
                                ErrMsg.ShowOnPageLoad = True
                                Exit Sub
                            End If
                        End If
    ...
    Protected Sub BtnDel_Click(sender As Object, e As EventArgs) Handles BtnDel.Click
        ...
        With CmdEdit
            .Connection = Conn
            .CommandType = CommandType.Text
            .CommandText = "UPDATE PdHdr SET ApprovedByDP=@P1,TimeApprovedDP=@P2,ApprovedByTBP=@P3,TimeApprovedTBP=@P4 WHERE NoPD=@P5"
            ...
            .Parameters.AddWithValue("@P5", NoPD)
            .ExecuteNonQuery()
            .Dispose()
        End With
