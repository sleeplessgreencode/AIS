Public Class FrmPayPDRKD
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("User") = "" Then
            ...
            Exit Sub
        Else
            If CheckAkses1(Session("User").ToString.Split("|")(1), "PayPDRKD") = False Then
                Response.Redirect("Default.aspx")
                Exit Sub
            End If
        End If
        ...
        Using CmdGrid As New Data.SqlClient.SqlCommand
            With CmdGrid
                ...
                .CommandText = "SELECT A.*, B.* FROM PdHdr A JOIN Job B ON A.JobNo=B.JobNo " + _
                               "WHERE A.JobNo IN (" + CSV + ") AND A.KSO=1 AND ApprovedByDK IS NOT NULL AND TglBayar IS NULL " + GetFilter1() + GetFilter2() + " ORDER BY TimeApprovedDK DESC"
            End With
    ...
    Private Sub BindGrid1()
        ...
        Using CmdGrid As New Data.SqlClient.SqlCommand
            With CmdGrid
                ...
                .CommandType = CommandType.Text
                .CommandText = "SELECT A.*, B.* FROM PdHdr A JOIN Job B ON A.JobNo=B.JobNo " + _
                               "WHERE A.JobNo IN (" + CSV + ") AND A.KSO=1 AND A.ApprovedByDK IS NOT NULL AND A.TglBayar IS NOT NULL " + GetFilter1() + GetFilter2() + " ORDER BY A.TglBayar DESC"
            End With
            Using DaGrid As New Data.SqlClient.SqlDataAdapter
                DaGrid.SelectCommand = CmdGrid
                Using DtGrid As New Data.DataTable
    ...
    Private Sub GridPD_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridPD.RowCommand
        If e.CommandName = "BtnPay" Then
            ...
            If SelectRecord.Cells(10).Text = "&nbsp;" Then
                Session("PD") = "UPD|" + SelectRecord.Cells(1).Text + "|FrmPayPDRKD.aspx"
            Else
                Session("PD") = "SEE|" + SelectRecord.Cells(1).Text + "|FrmPayPDRKD.aspx"
            End If
